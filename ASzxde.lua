...
--Properties:
-- [All properties are the same as before, omitted for brevity]
UESP1.Name = "UESP1"
...
ESPS.CanvasSize = UDim2.new(0, 0, 1.25, 0) 
...
TextButton_Aimbot.Active = true -- Make sure it's clickable
...
-- ===============================================
-- 1. AIMBOT GLOBAL SETTINGS AND TOGGLE LOGIC (FIXED)
-- ===============================================
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            if targetPart and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                -- Ally Check: ตรวจสอบสถานะปุ่ม Ally Check
                ...
                if distance < shortestDistance then
                    -- Basic FOV check (in world space distance for simplicity)
...
-- Aimbot Toggle Button Logic (FIXED to use MouseButton1Click)
TextButton_Aimbot.MouseButton1Click:Connect(function()
    if TextButton_Aimbot.BackgroundColor3 == Color3.fromRGB(47, 47, 47) then
        -- Toggle ON
        TextButton_Aimbot.BackgroundColor3 = ColorSwitch.ImageColor3
        TextButton_Aimbot.Parent.BackgroundColor3 = ColorSwitch.ImageColor3
        TextButton_Aimbot.Parent.BackgroundTransparency = 0.7
        aimbotSettings.Enabled = true
    else
        -- Toggle OFF
        TextButton_Aimbot.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
        TextButton_Aimbot.Parent.BackgroundColor3 = Color3.fromRGB(27, 27, 27)
        TextButton_Aimbot.Parent.BackgroundTransparency = 0
...
-- ===============================================
-- 2. GENERIC TOGGLE AND CORE UI SCRIPTS
-- ===============================================

-- Generic Toggle Function for all other ESP buttons
local function create_toggle_script(button)
    button.MouseButton1Click:Connect(function()
        if button.BackgroundColor3 == Color3.fromRGB(47, 47, 47) then
            -- Turn ON
            button.BackgroundColor3 = ColorSwitch.ImageColor3
            button.Parent.BackgroundColor3 = ColorSwitch.ImageColor3
            button.Parent.BackgroundTransparency = 0.7
        else
            -- Turn OFF
            button.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
            button.Parent.BackgroundColor3 = Color3.fromRGB(27, 27, 27)
            button.Parent.BackgroundTransparency = 0
        end
    end)
end

-- Apply generic toggle to existing buttons (excluding Aimbot, which is already done)
create_toggle_script(TextButton) -- Name ESP
create_toggle_script(TextButton_2) -- Health ESP
create_toggle_script(TextButton_3) -- Distance ESP
create_toggle_script(TextButton_4) -- Box ESP
create_toggle_script(TextButton_5) -- Outline/Glow ESP
create_toggle_script(TextButton_6) -- Tracer ESP
create_toggle_script(TextButton_7) -- Team Color
create_toggle_script(TextButton_8) -- Ally Check

-- Drag Functionality (Re-added for user experience)
...
-- GUI Open/Close Toggle (PC)
TextButton_9.MouseButton1Click:Connect(function()
    ESPFRAME.Visible = not ESPFRAME.Visible
    LTXOPEN.Visible = not ESPFRAME.Visible
end)

-- GUI Open/Close Toggle (Mobile - This button is in the main GUI)
BUTTONDESIGN_2.MouseButton1Click:Connect(function()
    ESPFRAME.Visible = not ESPFRAME.Visible
    LTXOPEN.Visible = not ESPFRAME.Visible
end)

-- LTXOPEN Button (The hidden button to re-open the GUI)
LTXOPEN.MouseButton1Click:Connect(function()
    LTXOPEN.Visible = false
    ESPFRAME.Visible = true
end)

-- Settings Toggle (Settingsbuttopn)
Settingsbuttopn.MouseButton1Click:Connect(function()
    SettingFrame.Visible = not SettingFrame.Visible
end)

-- Initial state for the main GUI
ESPFRAME.Visible = true
LTXOPEN.Visible = false 

-- Note: The logic for ESPCOLOR/colorpi (Color Picker) is highly complex and environment-dependent, 
-- and has been omitted to prioritize fixing the Aimbot/Toggle clicks.
-- However, ColorSwitch is initialized, allowing all toggles to use the default blue color.
